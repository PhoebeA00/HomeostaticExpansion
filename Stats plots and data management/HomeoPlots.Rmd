...
```{r}
popTregs2 = subset(popActiv, select = c("Age", "X4TregProlCT", "NaiveDerivedTregsCT", "X4TregFromThymusCT", "X4TregCT"))
popTregs2$X4TregProlCT = popTregs2$X4TregProlCT * 10**6
popTregs2$X4TregCT = popTregs2$X4TregCT * 10**6
...
ggplot(longpopTregs2, aes(x = Age, y = value, color = variable)) + 
  ...
  labs(titles = "Different Treg sources", x = "Age in Days", y = "Treg counts")+
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 2),
                 legend.key = element_rect(fill = "white", colour = "black"),
                 legend.background = (element_rect(colour= "black", fill = "white")),
        axis.title.x = element_text(face="bold", colour="black", size=20),
...
```{r}
popa = subset(pop, Age < 40)
ggplot(popa, aes(x = Age, y = X4TregProlCT, color = Genotype)) + geom_point()+
  geom_point(position = position_dodge(1), size = 4)
```
