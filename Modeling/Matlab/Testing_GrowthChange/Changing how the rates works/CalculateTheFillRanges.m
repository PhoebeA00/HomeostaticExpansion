function StatsOfCells = CalculateTheFillRanges(CellularData)
%{
...
StatsOfCells = zeros(length(rows), 6, 11, 2);
...
for gene = Genotype
    for pg = Pages
        for row = rows
            ...
            % (6) - Highest 90 percentile
            StatsOfCells(row,6,pg,gene) = prctile(CellularData(row, : , pg, gene), 10);
        end
    end
end