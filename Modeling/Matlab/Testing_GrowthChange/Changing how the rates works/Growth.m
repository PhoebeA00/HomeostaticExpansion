...
dNdt = mu - beta*N*(1/(1+((R*(I/(Ki+I)))/kA)^n)) + z*N - c - g*N;
dNproldt = z*N - beta*Nprol*(1/(1+((R*(I/(Ki+I)))/kA)^n)) - c - g*Nprol ; %Self replicating naive T cells
dThyNdt = mu - beta*ThyN*(1/(1+((R*(I/(Ki+I)))/kA)^n)) - c - g*ThyN; %Thymic derived naive T cells
...
dRdt = alpha + epsilon*R + c - b_R*R*(1/(1+(I/kB)^n1));
dThyRdt = alpha - b_R*ThyR*(1/(1+(I/kB)^n1)); %Thymic Dervied Tregs
dRproldt = epsilon*R - b_R*Rprol*(1/(1+(I/kB)^n1)); %Self Replicating Tregs
dDiffRdt = c - b_R*DiffR*(1/(1+(I/kB)^n1)); %Naive Derived Tregs
