...
dNdt = (mu*(1-(N/nK))) - beta*N*(1/(1+((R*(I/(Ki+I)))/kA)^n)) + z*N - c*N - g*N;
dNproldt = z*N - beta*Nprol*(1/(1+((R*(I/(Ki+I)))/kA)^n)) - c*Nprol - g*Nprol ; %Self replicating naive T cells
dThyNdt = (mu*(1-(N/nK))) - beta*ThyN*(1/(1+((R*(I/(Ki+I)))/kA)^n)) - c*ThyN- g*ThyN; %Thymic derived naive T cells
...
dRdt = (alpha*(1-(R/rK))) + epsilon*R + c*N - b_R*R*(1/(1+(I/kB)^n1));
dThyRdt = (alpha*(1-(R/rK))) - b_R*ThyR*(1/(1+(I/kB)^n1)); %Thymic Dervied Tregs
