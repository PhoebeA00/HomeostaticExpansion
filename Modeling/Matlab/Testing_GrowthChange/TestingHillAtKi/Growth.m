...
dNdt = (mu*N*(1-(N/nK))) - beta*N*(1/(1+((R)/(kA*((Ki+I)/I)))^n)) + z*N - c*N - g*N;
dNproldt = z*N - beta*Nprol*(1/(1+((R)/(kA*((Ki+I)/I)))^n)) - c*Nprol - g*Nprol ; %Self replicating naive T cells
...
dTdt = beta*N*(1/(1+((R)/(kA*((Ki+I)/I)))^n)) + a*T - j*R*T*(I/(Kj+I)) - b_T*T;
dActNdt = beta*N*(1/(1+((R)/(kA*((Ki+I)/I)))^n)) - j*R*ActN*(I/(Kj+I)) - b_T*ActN; %Activated Naive T cells
